...
class _ExerciseScreenState extends ConsumerState<ExerciseScreen> {
  ...
  Future<void> _generateNextQuestion() async {
    ...
    try {
      ...
      await repository.initialize(const LlmConfig(modelPath: 'dummy.gguf'));

      // Step 1: Generate English question ONLY (no translation - let the model focus on one task)
      const systemPrompt = 'You are an IT English quiz writer. Output JSON only. '
          'Format: {"question": "..."}';
      ...
      // AI generation fallback handled silently
    }
  ...
  Widget build(BuildContext context) {
    ...
    // If model not ready, display download card
    Widget modelDownloadCard = downloadState.status != ModelDownloadStatus.ready
        ? Container(
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: const [
                      Text(
                        'On-Device AI Model (Qwen2.5-Coder ~398MB)',
  ...
  Widget _buildSetupScreen(ThemeData theme) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('IT English Drills'),
      ),
      body: SafeArea(
        child: LayoutBuilder(
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const Gap(20),
          const Text(
            'Generating new IT English drill with AI...',
            style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold),
          ),
          const Gap(8),
          Text(
            'Creating real-world Bridge SE scenario & choices',
